...
def reconstruct(activity, positions, categorize):
    ...
    for cid in sorted(set(grouped) | set(snapshots)):
        ...
        for event in sorted(events, key=lambda e: e.get("timestamp") or 0):
            kind = event.get("type")
            if kind in ("REWARD", "MAKER_REBATE"):
        ...
        for key in set(balances) | set(pos):
            ...
            if snapshot is not None:
                try:
                    ...
                    if abs(balance - size) > max(EPS, abs(size) * 1e-6):
        ...
        metadata = next((e for e in events if e.get("title") or e.get("slug")),
                        current[0] if current else {})
        pnl = received + final_value - spent
        slug = metadata.get("eventSlug") or metadata.get("slug")
        rows.append({
            "conditionId": cid, "title": metadata.get("title"), "icon": metadata.get("icon"),
            "slug": slug, "category": categorize(slug, metadata.get("title")),
            ...
            "resolvedAt": max((int(e.get("timestamp") or 0) for e in events), default=0),
        })

    closed = [r for r in rows if r["settled"]]
    wins = sum(r["won"] for r in closed)
    held = sum(r["heldToResolution"] for r in closed)
    ...
    return {
        ...
        "hold_ratio": round(held / len(closed), 4) if closed else None,
        "avg_entry_price": round(total_buys / total_shares, 4) if total_shares else 0.0,
        "held": held, "exited": len(closed) - held, "per_market": rows,
